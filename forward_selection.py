...
def select_features(dset, target, tol = 0.001):
    # get the data
    trainset, set2,_ , eids_train, eids_set2, _, _ = get_data({'dset': dset, 'target': target, "combine_sets" : False})
    X = trainset[0]
    # y = trainset[1]
    X_val = set2[0]
    # y_val = set2[1]
...
def main():
    ...
    select_features(args.dset,args.target, args.tol)
